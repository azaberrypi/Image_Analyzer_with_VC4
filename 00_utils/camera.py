...
def capture2PIL(camera, stream, cap_size):
  camera.capture(stream, format='rgba', use_video_port=True) # rgb -> rgba
  raw_img = stream.getvalue()
  pil_img = Image.frombytes('RGBA', (cap_size[0], cap_size[1]), raw_img, 'raw') # RGB -> RGBA
...
class PiCameraOverlay:
  ...
  def OnOverlayUpdated(self, anOverlayImg, format='rgb', fullscreen=True, window=None): # fullscreen=False , windowsに値4つのタプル
